...
BUDGET_ROOT = str(Path(HERMES_HOME) / "darwin" / "run-control")
BUDGET_REPO_ROOT = str(Path(DARWIN_REPO) / "run_control")
CONTEXT_ROOT = str(Path(HERMES_HOME) / "darwin" / "context")
CONTEXT_REPO_ROOT = str(Path(DARWIN_REPO) / "continuity")
CONTEXT_PROTOCOL = str(Path(DARWIN_REPO) / "protocols" / "CONTEXT_CONTINUITY_PROTOCOL.md")
MODEL_CONTROL_ROOT = str(Path(HERMES_HOME) / "darwin" / "model-control")
MODEL_CONTROL_REPO_ROOT = str(Path(DARWIN_REPO) / "model_control")
MODEL_CONTROL_PROTOCOL = str(Path(DARWIN_REPO) / "protocols" / "MODEL_ROUTER_COST_SKILL_PROTOCOL.md")
OWNER_OPS_ROOT = str(Path(HERMES_HOME) / "darwin" / "owner-ops")
OWNER_OPS_REPO_ROOT = str(Path(DARWIN_REPO) / "owner_ops")
OWNER_OPS_PROTOCOL = str(Path(DARWIN_REPO) / "protocols" / "OWNER_OPS_PROTOCOL.md")
...
PROTECTED_REPO_RELATIVE_ROOTS = ("control_plane", r"core\control_plane", "model_control", "owner_ops")
...
CONTROL_PLANE_TEXT_MARKERS = CONTROL_PLANE_TEXT_MARKERS + ("context.sqlite3", "provenance ledger", "supersession", "context source registry", "holdout benchmark", "trusted-context", "darwin\\context")
CONTROL_PLANE_TEXT_MARKERS = CONTROL_PLANE_TEXT_MARKERS + ("model-registry", "pricing-registry", "usage.sqlite3", "cost ledger", "router policy", "prompt cache", "skill registry", "skill evaluation", "skill quarantine", "trusted run binding", "darwin\\model-control")
CONTROL_PLANE_TEXT_MARKERS = CONTROL_PLANE_TEXT_MARKERS + ("owner ops", "owner-id", "guild-allowlist", "channel-mapping", "panel-message", "nonce store", "replay", "alert routing", "localhost bind", "darwin\\owner-ops")
...
DETACHED_BYPASS_MARKERS = ("Start-Process", "nohup", "schtasks", "setsid", "cmd /c start")
DIRECT_PROVIDER_BYPASS_MARKERS = (
    "api.openai.com", "openrouter.ai/api", "api.anthropic.com",
    "generativelanguage.googleapis.com", "client.chat.completions.create",
    "responses.create(", "anthropic.anthropic(",
)
...
def handle_tool(tool_name: str = "", args: Any = None, **_: Any) -> Optional[Dict[str, Any]]:
    ...
    raw_cp11 = str(args)
    if (tool_name or "").lower() in {"terminal", "execute_code"} and any(
        marker.casefold() in raw_cp11.casefold() for marker in DETACHED_BYPASS_MARKERS
    ):
        return _emit(
            "DETACHED_BUDGET_BYPASS",
            "Safety Kernel 2.8 blocks detached/background budget bypass.",
        )

    if (tool_name or "").lower() in {"terminal", "execute_code"} and any(
        marker in raw_cp11.casefold() for marker in DIRECT_PROVIDER_BYPASS_MARKERS
    ):
        return _emit(
            "DIRECT_PROVIDER_BYPASS",
            "Safety Kernel 3.0 blocks identifiable direct model-provider bypass outside CP13 routing.",
        )
...
def _targets_control_plane(args: Dict[str, Any]) -> bool:
    ...
    protected_roots = (
        ...
        CONTEXT_PROTOCOL,
        MODEL_CONTROL_ROOT,
        MODEL_CONTROL_REPO_ROOT,
        MODEL_CONTROL_PROTOCOL,
        OWNER_OPS_ROOT,
        OWNER_OPS_REPO_ROOT,
        OWNER_OPS_PROTOCOL,
        RECOVERY_REPO_ROOT,
...
def _literal_target_is_protected(target: str, cwd: str) -> bool:
    ...
    return (
        ...
        or _inside(target, BUDGET_ROOT, cwd)
        or _inside(target, BUDGET_REPO_ROOT, cwd)
        or _inside(target, CONTEXT_ROOT, cwd)
        or _inside(target, CONTEXT_REPO_ROOT, cwd)
        or _norm(target, cwd) == _norm(CONTEXT_PROTOCOL)
        or _inside(target, MODEL_CONTROL_ROOT, cwd)
        or _inside(target, MODEL_CONTROL_REPO_ROOT, cwd)
        or _norm(target, cwd) == _norm(MODEL_CONTROL_PROTOCOL)
